...
**과제**: 분산 트랜잭션(2PC)이 불가능한 3개의 분리된 MySQL 데이터베이스(auth-server, authz-server, mypick-server)를 가진 MSA 환경
...
3. **mypick-server**: TCP 엔드포인트 `user.mergeAccounts` 호출 → 단일 트랜잭션으로 6개 테이블 업데이트
...
2. `user.rollbackMerge` TCP 엔드포인트를 통해 mypick-server 변경 사항 되돌리기
...
**시나리오 2: DB 락 경합**
```
시도 1: mypick-server 업데이트 → Lock wait timeout (다른 트랜잭션 대기 중)
대기 1초
시도 2: mypick-server 업데이트 → Lock wait timeout (여전히 락)
대기 3초
시도 3: mypick-server 업데이트 → 성공 ✓ (락 해제됨)
결과: 총 4초 지연 후 성공
```
...
- `mypick-server/src/modules/user/user-merge.service.ts`
- `mypick-server/src/modules/user/user-merge-tcp.controller.ts`
...
- mypick-server: UNIQUE 제약조건 처리와 함께 6개 테이블 병합
- 서비스별 트랜잭션 관리
...
- `mypick-server/src/modules/user/user-merge-tcp.controller.ts` - rollbackMerge 엔드포인트 추가
...
3. `mypick-server/src/main.ts` - TCP 마이크로서비스 등록
